...
【蝴蝶忍教学】自身HP2.5禁止硬抗大范围爆发·药学回血留至残血(0.5)使用·蜂牙真曳绕后命中脖颈触发斩杀·新人管理血量勿无脑连放技能+利用地形障碍+AD攻击命中脖颈才能直接斩杀`;
    ...
    function getSmartGreeting() {
        const totalGames = window.BattleRecords ? BattleRecords.count() : 0;
        const records = window.BattleRecords ? BattleRecords.getRecent(20) : [];

        if (totalGames === 0) {
            return '初次见面~我是虫柱蝴蝶忍，欢迎来到鬼杀队训练场！请先创建房间或试试单人模式，让我看看你的实力哦。';
        }
        ...
        const winRate = Math.round((wins / Math.min(totalGames, records.length)) * 100);
        const avgTurns = records.length > 0 ? Math.round(totalTurns / records.length) : 0;

        // 检查最近一场距今多久
        const lastRecord = records[0];
        let timeAgo = '';
        if (lastRecord && lastRecord.savedAt) {
            const diffMin = Math.round((Date.now() - new Date(lastRecord.savedAt).getTime()) / 60000);
            if (diffMin < 60) timeAgo = `${diffMin}分钟前`;
            else if (diffMin < 1440) timeAgo = `${Math.round(diffMin / 60)}小时前`;
            else timeAgo = `${Math.round(diffMin / 1440)}天前`;
        }

        // 根据数据生成不同问候
        if (totalGames <= 2) {
            return `又见面了~你才打了${totalGames}场，还在熟悉阶段呢。有什么不懂的尽管问我，我会耐心教你的。`;
        } else if (winRate >= 70) {
            return `${timeAgo ? timeAgo + ' ' : ''}欢迎回来！${totalGames}场战斗胜率${winRate}%，非常出色呢~看来你已经很熟练了。想挑战更高难度或者研究新战术吗？`;
        } else if (winRate >= 40) {
            return `${timeAgo ? timeAgo + ' ' : ''}回来啦~${totalGames}场胜率${winRate}%，均${avgTurns}回合。稳步提升中呢。需要我帮你分析哪些地方可以改进吗？`;
        } else {
            return `${timeAgo ? timeAgo + ' ' : ''}回来修炼了呀。${totalGames}场胜率${winRate}%，还有提升空间哦。要不要我帮你看看哪里可以优化？`;
        }
    }
    ...
    return {
        ...
        getGreeting() { return getSmartGreeting(); },
        clearHistory,
    };
})();
